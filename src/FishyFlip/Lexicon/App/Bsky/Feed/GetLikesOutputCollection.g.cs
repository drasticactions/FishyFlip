...
namespace FishyFlip.Lexicon.App.Bsky.Feed
{
    ...
    public class GetLikesOutputCollection : ATObjectCollectionBase<FishyFlip.Lexicon.App.Bsky.Feed.LikeDef>, IAsyncEnumerable<FishyFlip.Lexicon.App.Bsky.Feed.LikeDef>
    {
        ...
        /// <inheritdoc/>
        public override async Task<(IList<FishyFlip.Lexicon.App.Bsky.Feed.LikeDef> Posts, string Cursor)> GetRecordsAsync(int? limit = null, CancellationToken? token = default)
        {
            token = token ?? this.CancellationToken ?? System.Threading.CancellationToken.None;
            var (result, error) = await this.ATProtocol.GetLikesAsync(uri: this.Uri, cid: this.Cid, limit: limit, cursor: this.Cursor, cancellationToken: token.Value!);

            this.HandleATError(error);

            if (result == null || result.Likes == null)
            {
