...
public sealed class LoginViewController : UIViewController
{
    ...
    private UIButton getSessionButton;
    ...
    public LoginViewController()
    {
        ...
        this.getSessionButton = new UIButton(UIButtonType.System);
        this.getSessionButton.SetTitle("Get Logged In Session", UIControlState.Normal);
        this.getSessionButton.TouchUpInside += this.GetSessionButton_TouchUpInside;
        this.View!.AddSubview(this.getSessionButton);

        this.getSessionButton.TranslatesAutoresizingMaskIntoConstraints = false;
        this.View!.AddConstraints(new[]
        {
            NSLayoutConstraint.Create(this.getSessionButton, NSLayoutAttribute.Top, NSLayoutRelation.Equal, this.refreshTokenButton, NSLayoutAttribute.Bottom, 1, 20),
            NSLayoutConstraint.Create(this.getSessionButton, NSLayoutAttribute.Leading, NSLayoutRelation.Equal, this.View!, NSLayoutAttribute.Leading, 1, 20),
            NSLayoutConstraint.Create(this.getSessionButton, NSLayoutAttribute.Trailing, NSLayoutRelation.Equal, this.View!, NSLayoutAttribute.Trailing, 1, -20),
            NSLayoutConstraint.Create(this.getSessionButton, NSLayoutAttribute.Height, NSLayoutRelation.Equal, 1, 40),
        });
    }

    private async void GetSessionButton_TouchUpInside(object? sender, EventArgs e)
    {
        if (this.atProtocol.IsAuthenticated)
        {
            var (result, error) = await this.atProtocol.Server.GetSessionAsync();
            if (error != null)
            {
            ...
            else
            {
                this.InvokeOnMainThread(() =>
                {
                    var alert = UIAlertController.Create("Success", $"Session {result!.Did}.", UIAlertControllerStyle.Alert);
                    alert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, null));
                    this.PresentViewController(alert, true, null);
                });
            }
    ...
    private async void RefreshTokenButton_TouchUpInside(object? sender, EventArgs e)
    {
        if (this.atProtocol.IsAuthenticated)
        {
            var (result, error) = await this.atProtocol.SessionManager.RefreshSessionAsync();
            if (error != null)
            {
                this.InvokeOnMainThread(() =>
                {
            ...
            else
            {
                this.InvokeOnMainThread(() =>
                {
                    var alert = UIAlertController.Create("Success", $"Token refreshed. New access token: {result.AccessJwt}", UIAlertControllerStyle.Alert);
                    alert.AddAction(UIAlertAction.Create("OK", UIAlertActionStyle.Default, null));
                    this.PresentViewController(alert, true, null);
                });
            }
